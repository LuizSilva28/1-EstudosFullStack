...
function initApp() {
	...
	let allShips: (typeof spaceship)[] = [];

	function registerSpaceship(ship: typeof spaceship) {
		allShips.push(ship);
		console.log(allShips);
		alert(`Nave [ ${ship.name} ] cadastrada!`);
	}

	function addMember(allShips: (typeof spaceship)[]) {
		...
		const id = prompt(
			`Cadastro de tripulante:  ____________________________\n
               ${listShips} \n
		...
		if (id) {
			...
			if (ship && ship.crew !== undefined) {
				...
				let message =
					"Limite de tripulantes atingido, não é possível adicionar mais membros";
				alert(message);
				return;
	...
	function startMission() {
		...
		if (listShips.length > 0) {
			const id = prompt(
				`Definir missão:  ____________________________\n
            ${listShips} \n
            Digite a opção da NAVE: \n`,
				"1"
			);
			...
			if (ship && ship.crew) {
				let confirmationCrew: boolean =
					ship.crew.length >= Math.floor(ship.crewLimit / 3);
				...
				if (!ship.inMission && confirmationCrew) {
					const confirmationMission = prompt(
						`Iniciar missão:  ____________________________\n
					...
					const mission =
						prompt(`Definir missão:  ____________________________\n
                        Digite o nome da missão: `);

					const inMission =
						confirmationMission === "sim" ? true : false;
					...
					let message = `Tripulação insuficiente, adicione mais ${
						Math.floor(ship.crewLimit / 3) - ship.crew.length
					}!!`;
					alert(message);
					return;
				}
	...
	function createMenu() {
		const option = prompt(
			`BEM VINDO AO NOSSO CENTRO ESPACIAL! ________________________________________________\n
