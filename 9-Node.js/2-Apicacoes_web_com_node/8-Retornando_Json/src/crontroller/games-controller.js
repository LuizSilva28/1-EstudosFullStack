...
module.exports = {
	...
	show: (req, res) => {
		...
		if (!game) {
			res.status(404);
			res.json({ message: "Game not faund!" });
		} else {
			res.json(game);
		}
	},
	...
	addGenre: (req, res) => {
		...
		if (
			typeof genre !== "string" ||
			games[gameIndex].genres.includes(genre)
		) {
	...
	removeGenre: (req, res) => {
		...
		if (
			typeof name !== "string" ||
			!games[gameIndex].genres.includes(name)
		) {
			return res.status(404).json({ message: "Invalide genre!" });
		}
